//! Deserialization of operation results.
//!
//! Implement `TryFrom<&[u8]> for OperationResult` and helper parsers used by it.
//!
//! Each operation result is encoded with a leading type byte followed by
//! type-specific payload. The top-level conversion dispatches on that type
//! byte and delegates to specialized deserializers below.
...
/// Deserialize a (card_id, amount) pair used in account query results.
...
/// Deserialize an AccountQuery result.
///
/// Expected layout:
/// - account_id (ACC_ID_SRL_LEN)
/// - total_spent (AMOUNT_SRL_LEN)
/// - per_card_spent_len (PER_CARD_SPENT_LEN as usize)
/// - per_card_spent entries: each CARD_SPENT_LEN bytes
...
/// Deserialize a LimitCard result.
///
/// The first byte indicates OK or ERR. Error payload handling is TODO.
...
/// Deserialize a LimitCheckError from its byte encoding.
///
/// Validate that sufficient bytes are present and map the code to the enum.
fn deserialize_limit_check_error(payload: &[u8]) -> AppResult<LimitCheckError> {
    if payload.len() < LIMIT_CHECK_ERR_LEN {
        return Err(AppError::InvalidProtocol {
            details: format!("not enough bytes to deserialize limit_check_error: {payload:?}"),
        });
    }
...
/// Deserialize a LimitUpdateError code.
fn deserialize_limit_update_error(payload: &[u8]) -> AppResult<LimitUpdateError> {
    match payload[0] {
        ...
        _ => Err(AppError::InvalidProtocol {
            details: format!("unknown limit_update_error: {payload:?}"),
        }),
    }
}

/// Deserialize a VerifyError which can be either a charge-limit or limit-update error.
fn deserialize_verify_error(payload: &[u8]) -> AppResult<VerifyError> {
    match payload[0] {
        ...
        _ => Err(AppError::InvalidProtocol {
            details: format!("unknown verify error code: {payload:?}"),
        }),
    }
}

/// Deserialize a LimitAccount result.
///
/// First byte indicates OK or ERR. If ERR, parse the following VerifyError payload.
...
mod test {
    ...
    fn test_deserialize_account_query_result() {
        let op_result = OperationResult::AccountQuery(AccountQueryResult {
            ...
            per_card_spent: vec![(15300, 8503.4), (6003, 18443.9), (203, 503.0)],
        });
